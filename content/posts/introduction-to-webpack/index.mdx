...
`Webpack-dev-server` recompile when changes are made in our code, but it doesn’t rebuild it by default. To fix this we need to tell `webpack-dev-server` where to output thee rebuild code.
